...
class Mp
{
	...
	//const API_URL_PREFIX = 'https://140.206.160.101/cgi-bin';
	...
	public function __construct($options=null)
	{
		...
		$this->fromtype = isset($options['fromtype'])?$options['fromtype']:'open';
	}
	...
	public function valid($return=false)
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            if ($this->encrypt_type == 'aes') { //aes加密
            	...
            	$pc = new MPrpcrypt($this->encodingAesKey);
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	private function http_get($url){
		...
		//写日志
		$logkey = LogKey::log_wxopen_api();
		$logdata = array();
		$logdata['request']= (string)$url;
		$logdata['method']= 'GET';
		$logdata['appid']= $this->appid;
		$logdata['version']= $this->fromtype;
		$logdata['http_code']=(string)json_encode($aStatus);
		$logdata['useragent']= $_SERVER['HTTP_USER_AGENT']?$_SERVER['HTTP_USER_AGENT']:"curl";
		$logdata['response']=(string)$sContent;
		$logdata['server'] = json_encode($_SERVER);
		PubFun::write_log($logkey,$logdata);
		curl_close($oCurl);
	...
	protected function setCache($cachename,$value,$expired){
		...
		cache_set($cachename, $value,$expired);
		return true;
	}
	...
	protected function getCache($cachename){
		...
		$value = cache_get($cachename);
		if(!is_array($value)&&strlen($value)>20){
		    return $value;
		}else{
		    return false;
		}
	...
	public function checkAuth($appid='',$appsecret='',$token=''){
		...
		$authname = 'wechat_access_token:'.$appid.'-from-'.$this->fromtype;
	...
	public function resetJsTicket($appid=''){
		if (!$appid) $appid = $this->appid;
		$this->jsapi_ticket = '';
		$authname = CacheKey::get_wechat_jsapi_ticket($appid.'-from-'.$this->fromtype);
		$this->removeCache($authname);
		return true;
	}
	...
	public function getJsTicket($appid='',$jsapi_ticket=''){
		...
		if ($result)
		{
			...
			//$expire = $json['expires_in'] ? intval($json['expires_in'])-100 : 3600;
			$this->setCache($authname,$this->jsapi_ticket,3600);
			return $this->jsapi_ticket;
		}
		return false;
	}
	...
	public function getJsSign($url, $timestamp=0, $noncestr='', $appid=''){
	    ...
	    if (!$sign)
	        return false;
	    $tar = strpos($url,'tar_foid');//去除tar_foid
	    if($tar){
	        $url = substr($url,0,$tar);
	    }
	    $signPackage = array(
	            "appid"     => $this->appid,
	            "noncestr"  => $noncestr,
...
//        s($result);
	...
	/**
	 * 上传图文消息中的图片
	 * @param $data array('media'=>"@test.jpg")
	 * @return array|bool|mixed|stdClassshangc
	 */
	public function uploadImg($data){
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::UPLOAD_MEDIA_URL.self::MEDIA_IMG_UPLOAD.'access_token='.$this->access_token,self::json_encode($data));
		if ($result)
		{
			$json = json_decode($result,true);
			if (!$json || !empty($json['errcode'])) {
	...
	public function batchGetUserInfo($openids){
	    ...
	    foreach ($openids as $openid){
	        $temp = array();
	        $temp["openid"] = $openid;
	        array_push($data["user_list"],$temp);
	        unset($temp);
	    }
	    if (!$this->access_token && !$this->checkAuth()) return false;
	    $result = $this->http_post(self::API_URL_PREFIX.self::USER_INFO_BATCHGET_URL.'access_token='.$this->access_token, self::json_encode($data));
	    if ($result)
	    {
	        $json = json_decode($result,true);
	        if (isset($json['errcode'])) {
	 ...
	 * @return boolean|array
	 */
	public function batchUpdateTagMembers($tagid,$openid_list){
		if (!$this->access_token && !$this->checkAuth()) return false;
		$data = array(
				'openid_list'=>$openid_list,
				'tagid'=>$tagid
		);
		$result = $this->http_post(self::API_URL_PREFIX.self::TAG_MEMBER_BATCHUPDATE_URL.'access_token='.$this->access_token,self::json_encode($data));
		if ($result)
		{
			$json = json_decode($result,true);
			if (!$json || !empty($json['errcode'])) {
    ...
    public function userTagGet($tagid,$next_openid){
		if (!$this->access_token && !$this->checkAuth()) return false;
		$data = array(
				'tagid'=>$tagid,
				'next_openid'=>$next_openid
		);
		$result = $this->http_post(self::API_URL_PREFIX.self::USER_TAG_GET_URL.'access_token='.$this->access_token,self::json_encode($data));
		if ($result)
		{
			$json = json_decode($result,true);
			if (!$json || !empty($json['errcode'])) {
	...
	public function sendTemplateMessage($data){
		...
		if($result){
			$json = json_decode($result,true);
			/*if(class_exists('Model_WxTplLog')){
				$tpllog = new Model_WxTplLog();
				$tpllogdata = array();
				$tpllogdata['openid'] = $data['touser'];
				$tpllogdata['wxtpl_id'] = $data['template_id'];
				$tpllogdata['url'] = $data['url'];
				$tpllogdata['send_data'] = json_encode($data);
				$tpllogdata['result_data'] = $result;
				if(!empty($json['errcode'])){
					$tpllogdata['status'] = -1;
				}else{
					$tpllogdata['status'] = 1;
				}
				$tpllog->insert_data($tpllogdata);
				$tpllog->close();
			}*/
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
...
class MPKCS7Encoder
{
    ...
    {
        $block_size = MPKCS7Encoder::$block_size;
        $text_length = strlen($text);
        //计算需要填充的位数
        $amount_to_pad = MPKCS7Encoder::$block_size - ($text_length % MPKCS7Encoder::$block_size);
        if ($amount_to_pad == 0) {
            $amount_to_pad = MPKCS7Encoder::block_size;
        }
        //获得补位所用的字符
    ...
    function decode($text)
    {
        ...
        if ($pad < 1 || $pad > MPKCS7Encoder::$block_size) {
            $pad = 0;
        }
...
class MPrpcrypt
{
    ...
    public function encrypt($text, $appid)
    {
        ...
        try {
            ...
            return array(MpErrorCode::$OK, base64_encode($encrypted));
        } catch (Exception $e) {
            //print $e;
            return array(MpErrorCode::$EncryptAESError, null);
        }
    }
    ...
    public function decrypt($encrypted, $appid)
    {
        ...
        try {
            ...
            return array(MpErrorCode::$DecryptAESError, null);
        }
        ...
        try {
            ...
            //print $e;
            return array(MpErrorCode::$IllegalBuffer, null);
        }
        if ($from_appid != $appid)
            return array(MpErrorCode::$ValidateAppidError, null);
